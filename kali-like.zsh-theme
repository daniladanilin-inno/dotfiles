...
FGPROMPT_USER=196          # COLOR - user@host text (blood red)
FGPROMPT_ROOT=196          # COLOR - user@host text when running as root (blood red)
FRAMEPROMPT_USER=196       # COLOR - prompt frame (┌─ ─ └─ brackets) (blood red)
FRAMEPROMPT_ROOT=196       # COLOR - prompt frame when running as root (blood red)
VENVPROMPT_COLOR=154      # COLOR - virtualenv / conda env name
VCSPROMPT_COLOR=154       # COLOR - git branch / repo info
SSHPROMPT_COLOR=cyan       # COLOR - SSH indicator
TMUXPROMPT_COLOR=magenta   # COLOR - tmux indicator
DOCKERPROMPT_COLOR=cyan    # COLOR - container indicator
NIXPROMPT_COLOR=magenta    # COLOR - nix indicator
K8SPROMPT_COLOR=cyan       # COLOR - kubectl current context
CMDTIME_COLOR=magenta      # COLOR - command execution time (RPROMPT)
...
PATHPROMPT_COLOR=154       # COLOR - current directory path
# Number of ─ characters after the initial ┌─
PROMPT_DASH_COUNT=1
# Prompt symbol after the frame (">" = BackTrack style; try "$" or "")
PROMPT_SYMBOL='>'
...
# (agnoster keeps the same segment colors in light mode — only text flips to white)
if [[ "$_resolved_theme_mode" == light ]]; then
    FGPROMPT_USER=196
    FGPROMPT_ROOT=196
    FRAMEPROMPT_USER=196
    FRAMEPROMPT_ROOT=196
    VENVPROMPT_COLOR=154
    VCSPROMPT_COLOR=154
    SSHPROMPT_COLOR=cyan
    TMUXPROMPT_COLOR=magenta
    DOCKERPROMPT_COLOR=cyan
    NIXPROMPT_COLOR=magenta
    K8SPROMPT_COLOR=cyan
    CMDTIME_COLOR=magenta
...
configure_prompt() {
    zstyle ':vcs_info:*' formats "%F{$VCSPROMPT_COLOR}[%b%c%u%F{$VCSPROMPT_COLOR}] %f"
    zstyle ':vcs_info:*' actionformats "%F{$VCSPROMPT_COLOR}[%b|%a%c%u%F{$VCSPROMPT_COLOR}] %f"

    if [[ $UID == 0 || $EUID == 0 ]]; then
        FGPROMPT="%F{$FGPROMPT_ROOT}"
        FRAMEPROMPT="%F{$FRAMEPROMPT_ROOT}"
    else
